...
def manhattan_distance(pointA, pointB):
    return sum(map(lambda (a,b): abs(a-b), zip(pointA, pointB)))
...
class KDTreeNeighbours():
    ...
    def add(self, point):
        sd = manhattan_distance(point, self.query_point)
...
class KDTree():
    ...
    def __init__(self, data):
        def build_kdtree(point_list, depth, k=False):
            ...
            if k is False:
              k = len(point_list[0])
            axis = depth % k # assumes all points have the same dimension
            ...
            node = KDTreeNode(point=point_list[median],
                              left=build_kdtree(point_list[0:median], depth+1, k),
                              right=build_kdtree(point_list[median+1:], depth+1, k))
            return node
